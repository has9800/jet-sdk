...
class DatasetBuilder:
    def __init__(self, source: str, split: str = "train", text_field: str | None = None,
                 input_field: str | None = None, target_field: str | None = None,
                 streaming: bool = False, max_samples: int | None = None):
    ...
    def load(self):
        ...
        if kind == "json": ds = load_dataset("json", data_files=loc, **kwargs)
        elif kind == "csv": ds = load_dataset("csv", data_files=loc, **kwargs)
        elif kind == "parquet": ds = load_dataset("parquet", data_files=loc, **kwargs)
        elif kind == "text": ds = load_dataset("text", data_files=loc, **kwargs)
        else: ds = load_dataset(loc, **kwargs)
